  ...
  stream.on('data', function(tweet) {
  	if(tweet.lang.indexOf("en") > -1){
  		...
  		words.forEach(function(word){
  			...
  			var isHexColor = /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(word);
  			if (isHexColor){
  				...
  				hexColor = matchWordColor(word.replace("#", ""));
  			}
        ...
        // NEED TO SEND TWEET AND RGB INFO TO ELASTICACHE 
  		}
  		else{
  			console.log("Tweet did not contain a valid color");
  		}


  	}
    // console.log(utils.inspect(tweet));
  });
